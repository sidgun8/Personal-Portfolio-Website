...
const ExpandableChatToggle: React.FC<ExpandableChatToggleProps> = ({
  ...
  <button
    onClick={toggleChat}
    className={cn(
      "bg-primary text-secondary w-12 h-12 rounded-full shadow-md flex items-center justify-center hover:shadow-lg transition-all duration-300",
      className,
    )}
    {...props}
  >
    {isOpen ? (
      <X  strokeWidth={1.8} className="h-6 w-6" />
    ) : (
      icon || <BrainCircuit strokeWidth={1.8}  className="h-6 w-6" />
    )}
  </button>
);
